...
\textbf{Option 1: Using Homebrew LLVM (Recommended)}
...
# Clone and build
git clone https://github.com/Sapana-Micro-Software/MRFcompiler.git
cd MRFcompiler
make check-compiler  # Verify compiler detection
make
\end{lstlisting}

\textbf{Option 2: Using Homebrew GCC}
...
\textbf{Option 3: Using Xcode Command Line Tools}
...
\begin{itemize}
    \item Homebrew LLVM (if installed at \texttt{/opt/homebrew/opt/llvm} or \texttt{/usr/local/opt/llvm})
    \item Homebrew GCC (if installed, checks for g++-13, g++-12, g++-11)
    \item System clang (Apple's LLVM) as fallback
\end{itemize}
...
\begin{itemize}
    \item \texttt{<node_id>}: The node ID for which the CPT is defined
    \item \texttt{[parent_states...]}: For nodes with parents, specify the parent state combination (one integer per parent, in order of parent node IDs)
    \item \texttt{<prob_state0> <prob_state1> ...}: Probability values for each state of the node (must sum to 1.0 for each parent combination)
\end{itemize}

\subsubsection{Root Node CPTs}
...
This means: P(Node0=0) = 0.8, P(Node0=1) = 0.2

\subsubsection{Node with Parents CPTs}

For nodes with parents, you must specify a CPT entry for each parent state combination. Each line specifies one parent combination:
...
\subsubsection{CPT Example: Bayesian Network}
